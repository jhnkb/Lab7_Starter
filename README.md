...
4) 
   - According to Lighthouse "<html> element does not have a [lang] attribute." We can fix this by adding a language tag like lang="en" to the HTML element so browsers and screen readers know the language of the webpage.
   ...
   - According to Lighthouse, we should "Use efficient cache lifetimes." The caches are only being stored for 10 minutes it looks like. The site is not reusing the files but it is  redownloading them when someone is visiting multiple times. We could increase cache lifetime so that files are stored in the browser longer. This should reduce loading times. 
