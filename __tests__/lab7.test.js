describe('Basic user flow for Website', () => {
  ...
  it('Make sure <product-item> elements are populated', async () => {
    ...
    */
    ...
    for (let i = 1; i < 20; i++){
      ...
      if (value.title.length == 0) { allArePopulated = false; break;}
      if (value.price.length == 0) { allArePopulated = false; break;}
      if (value.image.length == 0) { allArePopulated = false; break;}
    }
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
     ...
     */
     ...
     let text = await innerProp.jsonValue();

     if (text === "Remove from Cart"){textChanged = true;}
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    let number = Number(await count.jsonValue());
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    let pageElements = await page.$$('product-item');

    for (let j = 0; j < pageElements.length; j++){
      let shadowRoot = await pageElements[j].getProperty('shadowRoot');
      let button = await shadowRoot.$('button');
      let innertext = await button.getProperty('innerText');
      let text = await innertext.jsonValue()

      if (text === "Remove from Cart"){textChanged = true;} else {textChanged = false; break;}
    }
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (let j = 0; j < pageElements.length; j++){
      ...
      if (text === "Add to Cart"){textChanged = true;} else {textChanged = false; break;}
    }
